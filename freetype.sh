...
7z x '-oC:\Temp\gcc' freetype-2.6.2.tar > /dev/null || fail_with freetype-2.6.2.tar - EPIC FAIL

cd /c/temp/gcc
mv freetype-2.6.2 src
mkdir build dest
cd build
...
cd /c/temp/gcc
rm -rf build src
mv dest freetype-2.6.2
cd freetype-2.6.2
rm -rf bin lib/pkgconfig lib/*.la share

7z -mx0 a ../freetype-2.6.2.7z *
