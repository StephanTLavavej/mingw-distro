...
7za x '-oC:\Temp\gcc' pngcrush-1.7.30-nolib.tar > NUL || { echo pngcrush-1.7.30-nolib.tar - EPIC FAIL ; exit 1; }
...
gcc -s -Os -fomit-frame-pointer pngcrush-1.7.30-nolib/pngcrush.c -o dest/bin/pngcrush.exe -lpng -lz || { echo pngcrush - EPIC FAIL ; exit 1; }

gcc -s -Os -fomit-frame-pointer pngrewrite-1.4.0/*.c -o dest/bin/pngrewrite.exe -lpng -lz || { echo pngrewrite - EPIC FAIL ; exit 1; }

rm -rf pngcheck-2.3.0 pngcrush-1.7.30-nolib pngrewrite-1.4.0
