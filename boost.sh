...
7z x '-oC:\Temp\gcc' boost_1_59_0.tar '-x!boost_1_59_0\libs\preprocessor\doc' > /dev/null \
|| fail_with boost_1_59_0.tar - EPIC FAIL
...
./b2 $X_B2_JOBS cxxflags="-std=c++14" pch=off --without-context --without-coroutine --without-coroutine2 \
variant=release link=static runtime-link=static threading=multi --stagedir=/c/temp/gcc/dest stage -sNO_BZIP2 \
-sBZIP2_BINARY=bz2 -sBZIP2_INCLUDE=$X_DISTRO_INC -sBZIP2_LIBPATH=$X_DISTRO_LIB -sNO_ZLIB -sZLIB_BINARY=z \
-sZLIB_INCLUDE=$X_DISTRO_INC -sZLIB_LIBPATH=$X_DISTRO_LIB || fail_with boost 2 - EPIC FAIL
