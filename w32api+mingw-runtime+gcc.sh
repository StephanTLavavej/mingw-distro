...
7za x '-oC:\Temp\gcc' gcc-core-4.6.3.tar > NUL || { echo gcc-core-4.6.3.tar - EPIC FAIL ; exit 1; }
7za x '-oC:\Temp\gcc' gcc-g++-4.6.3.tar > NUL || { echo gcc-g++-4.6.3.tar - EPIC FAIL ; exit 1; }
...
patch -d /c/temp/gcc/gcc-4.6.3 -p1 < gcc.patch

# http://gcc.gnu.org/viewcvs/trunk/libstdc%2B%2B-v3/libsupc%2B%2B/exception_ptr.h?view=log
patch -d /c/temp/gcc/gcc-4.6.3 -p1 < gcc-r174075.patch
...
make all install "CFLAGS=-s -Os -fomit-frame-pointer" || { echo gmp make - EPIC FAIL ; exit 1; }
cd /c/temp/gcc
rm -rf build src
...
make all install "CFLAGS=-s -Os -fomit-frame-pointer" || { echo mpfr make - EPIC FAIL ; exit 1; }
cd /c/temp/gcc
rm -rf build src
rm -rf dest/lib/*.la dest/share
mv dest mpfr
...
make all install "CFLAGS=-s -Os -fomit-frame-pointer" || { echo mpc make - EPIC FAIL ; exit 1; }
cd /c/temp/gcc
rm -rf build src
rm -rf dest/lib/*.la dest/share
mv dest mpc
...
make all install "CFLAGS=-Os -fomit-frame-pointer" "LDFLAGS=-s" || { echo w32api make - EPIC FAIL ; exit 1; }
cd /c/temp/gcc
rm -rf build src
mv dest w32api
...
make all install "CFLAGS=-Os -fomit-frame-pointer" "LDFLAGS=-s" || { echo mingw-runtime make - EPIC FAIL ; exit 1; }
cd /c/temp/gcc
