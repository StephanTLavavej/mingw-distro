...
7z x '-oC:\Temp\gcc' coreutils-8.22.tar > NUL || fail_with coreutils-8.22.tar - EPIC FAIL

patch -d /c/temp/gcc/coreutils-8.22 -p1 < coreutils.patch

cd /c/temp/gcc
mv coreutils-8.22 src
...
cd /c/temp/gcc
rm -rf build src
mv dest coreutils-8.22
cd coreutils-8.22

7z -mx0 a ../coreutils-8.22.7z *
